  ...
  void)
{
  ...
  _fido_dbus_obj = fido_proxy_new_sync (connection,
					...
					_FIDO_DBUS_NAME,
					_FIDO_DBUS_PATH, NULL, &error);
}

Fido *
_dbus_proxy_get_instance (
  int time_out)
{
...
static void
_fido_uaf_process_operation_reply (
  GObject * object,
  GAsyncResult * res,
  gpointer user_data)
{
  ...
  fido_call_fido_uaf_process_operation_finish (_fido_dbus_obj, &tizen_err,
					       &uaf_response_json, res,
					       &dbus_err);
  ...
  void *user_data)
{
  ...
  fido_call_fido_uaf_discover_sync (dbus_proxy, &tz_err, &discovery_data_json,
				    &discovery_data_json_list_len, NULL,
				    &dbus_err);

  if (dbus_err != NULL)
    {
      _ERR ("fido_call_fido_uaf_discover_sync failed [%s]",
	    dbus_err->message);
      g_clear_error (&dbus_err);
  ...
  int parser_err = 0;
  GList *auth_list =
    _uaf_parser_parse_asm_response_discover_client (discovery_data_json,
						    discovery_data_json_list_len,
						    &parser_err);
  if ((auth_list == NULL) && (parser_err != FIDO_ERROR_NONE))
    {
	  ...
	  while (auth_list_iter != NULL)
	    {
	      ...
	      fido_authenticator_s *auth_priv =
		(fido_authenticator_s *) (auth_list_iter->data);
...
fido_uaf_is_supported (
  const char *uaf_message_json,
  bool * is_supported)
{
  if (uaf_message_json == NULL)
    {
      _ERR
	("uaf_message_json can not be NULL [FIDO_ERROR_INVALID_PARAMETER]");
      return FIDO_ERROR_INVALID_PARAMETER;
    }
  ...
  if (dbus_err != NULL)
    {
      _ERR ("fido_call_fido_uaf_check_policy_sync failed [%s]",
	    dbus_err->message);
      g_clear_error (&dbus_err);
      return FIDO_ERROR_PERMISSION_DENIED;
    }
  ...
  void *user_data)
{
  ...
  if (uaf_request_json == NULL)
    {
      _ERR
	("uaf_request_json can not be NULL [FIDO_ERROR_INVALID_PARAMETER]");
      return FIDO_ERROR_INVALID_PARAMETER;
    }
  ...
  _fido_process_cb_data_s *cb_data =
    (_fido_process_cb_data_s *) calloc (1, sizeof (_fido_process_cb_data_s));
  cb_data->cb = callback;
  ...
  const char *uaf_response_json)
{
  if (uaf_response_json == NULL)
    {
      _ERR
	("uaf_response_json can not be NULL [FIDO_ERROR_INVALID_PARAMETER]");
      return FIDO_ERROR_INVALID_PARAMETER;
    }
  ...
  _response_t *uaf_res_data =
    _uaf_parser_parse_uaf_response (uaf_response_json);
  ...
  if (strcmp (uaf_res_data->header->operation, _UAF_OPERATION_NAME_KEY_REG) !=
      0)
    {
      _free_response (uaf_res_data);
      return FIDO_ERROR_NONE;
  ...
  GError *err = NULL;
  gboolean is_success =
    fido_call_fido_uaf_process_operation_sync (dbus_proxy, uaf_dereg_json,
					       _FIDO_NO_CHANNEL_BINDING_DBUS_STRING,
					       &tz_err, &resp, NULL, &err);

  if (is_success == FALSE)
    {
      _ERR ("fido_call_fido_uaf_notify_result_sync failed [%d]",
	    tizen_error_code);
      if (dbus_err)
	{
